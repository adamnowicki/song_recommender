    ...
    if chosen_track_id:
       ...
       'loudness', 'speechiness', 'acousticness', 'instrumentalness',
        ...
        if feat == 'danceability':
            ...
            with open('model_km_danc.pickle', 'rb') as handle:
                km_danc = pickle.load(handle)

            with open('scaler_danc.pickle', 'rb') as handle:
                scaler_danc = pickle.load(handle)
            
            scaled_song = scaler_danc.transform(song_w_df)
            prediction=km_danc.predict(scaled_song)

            clusters_df= pd.read_csv("tracks_clustered_df_da.csv")
            df_good=clusters_df[clusters_df['km_danc']==prediction[0]]
        ...
        # call pickle for each & run test
            ...
            with open('model_km_ener.pickle', 'rb') as handle:
                    km_ener = pickle.load(handle)

            with open('scaler_ener.pickle', 'rb') as handle:
            ...
            clusters_df= pd.read_csv("tracks_clustered_df_ener.csv")
            df_good=clusters_df[clusters_df['km_ener']==prediction[0]]
            suggestion=df_good['track_id'].sample(n=1).iloc[0]
        ...
        # call pickle for each & run test
            ...
            with open('model_km_sp.pickle', 'rb') as handle:
                km_sp = pickle.load(handle)

            with open('scaler_sp.pickle', 'rb') as handle:
            ...
            clusters_df= pd.read_csv("tracks_clustered_df_sp.csv")
            df_good=clusters_df[clusters_df['km_sp']==prediction[0]]
            suggestion=df_good['track_id'].sample(n=1).iloc[0]
        ...
        # call pickle for each & run test
            ...
            with open('model_km_ac.pickle', 'rb') as handle:
                km_ac = pickle.load(handle)

            with open('scaler_ac.pickle', 'rb') as handle:
                scaler_ac = pickle.load(handle)

            scaled_song_ac = scaler_ac.transform(song_w_df)
            prediction = km_ac.predict(scaled_song_ac)

            clusters_df= pd.read_csv("tracks_clustered_df_ac.csv")
            df_good=clusters_df[clusters_df['km_ac']==prediction[0]]
            suggestion=df_good['track_id'].sample(n=1).iloc[0]
        ...
        # call pickle for each & run test
            ...
            with open('model_km_ins.pickle', 'rb') as handle:
                km_ins = pickle.load(handle)
            with open('scaler_ins.pickle', 'rb') as handle:
            ...
            clusters_df= pd.read_csv("tracks_clustered_df_ins.csv")
        ...
        # call pickle for each & run test
            ...
            with open('model_km_val.pickle', 'rb') as handle:
                km_val = pickle.load(handle)

            with open('scaler_val.pickle', 'rb') as handle:
                scaler_val = pickle.load(handle)

            scaled_song_val = scaler_val.transform(song_w_df)
            prediction = km_val.predict(scaled_song_val)

            clusters_df= pd.read_csv("tracks_clustered_df_val.csv")
            df_good=clusters_df[clusters_df['km_val']==prediction[0]]
            suggestion=df_good['track_id'].sample(n=1).iloc[0]
        ...
        st.write(f'<iframe src="https://open.spotify.com/embed/track/{suggestion}?utm_source=generator" width="320" height="80" frameborder="0" allowtransparency="true" allow="encrypted-media"></iframe>', unsafe_allow_html=True)
